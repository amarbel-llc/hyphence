...
#
# regenerate go/gomod2nix.toml from go/go.mod and go/go.sum
[group('build')]
build-gomod2nix:
...
#
# run the flake checks: hermetic go-test plus the sandboxed formatting gate
[group('build')]
build-nix-check:
...
#
# run the Go test suite in the devShell
[group('post-build')]
test-go:
...
#
# run the Rust test suite in the devShell
[group('post-build')]
test-rust:
...
#
# run the hermetic CLI bats integration suite
[group('post-build')]
test-bats:
...
#
# validate docs/rfcs/hyphence-content.peg with langlang
[group('post-build')]
validate-grammar:
...
#
# cross-check the RFC test vectors against the content grammar
[group('post-build')]
...
#
# run the impure git-state eng-convention checks against the working tree
[group('pre-build')]
lint-worktree:
    #!/usr/bin/env bash
    set -euo pipefail
...
#
# tag a release and push the signed tag to origin
[group('maintenance')]
tag version message:
    #!/usr/bin/env bash
    set -euo pipefail
...
#
# cut a release: bump version.env, commit, push master, then push the tag
[group('maintenance')]
release version:
    #!/usr/bin/env bash
    set -euo pipefail
